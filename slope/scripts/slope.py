...
def create_points_fc(point_results, out_path, out_name, spatial_reference):
    ...
    cursor = arcpy.da.InsertCursor(geodb + out_name, ('PSRCEdgeID', 'SHAPE@XY'))
...
def process_elev_data(points_fc_with_elev, edges_fc):
    ...
    edges = arcpy.da.FeatureClassToNumPyArray(edges_fc, ('PSRCEdgeID', 'NewINode', 'NewJNode', 
        'ActiveLink', 'FacilityTy', 'Fullname', 'Direction', 'Shape_Length'))
...
def main(run_split_edges, run_process_elev_data, run_join_elev_data):
    ...
    if run_split_edges:
        ...
        create_points_fc(point_results=point_results, out_path=geodb,
    ...
    if run_process_elev_data:
        process_elev_data(points_fc_with_elev=points_fc_with_elev, 
            edges_fc=edges_fc)
