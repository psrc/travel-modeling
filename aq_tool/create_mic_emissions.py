...
# Calculate running emissions through MICs
...
output_dir = os.path.join(os.getcwd(), r'output/mic', model_year)


def evaluate_emissions(mic, _gdf_shp, taz_gdf, model_year, df_running_rates):
    ...
    city_gdf = gdf_shp[gdf_shp['mic'] == mic]
    ...
    #             city_gdf, 
    #             how="intersection")
    # # start_emissions_df = calculate_start_emissions(conn, parcel_intersect_gdf, model_year, df_veh, parcel_geog, df_hh, start_rates_df, df_bus_veh)
    ...
    summary_df['running_daily_tons'] = summary_df['interzonal_tons']+summary_df['intrazonal_tons']
    summary_df = summary_df[['pollutantID','pollutant_name','veh_type','intrazonal_vmt','interzonal_vmt','intrazonal_tons','interzonal_tons','running_daily_tons']]
...
version = "'DBO.Default'"
gdf_shp = read_from_sde(connection_string, 'micen', version, crs=crs, is_table=False)
...
for mic in gdf_shp['mic'].unique():
    if '/' in mic:
        fname = mic.replace('/', '_')
    else:
        fname = mic
    # if not os.path.isfile(os.path.join(output_dir,fname+'.csv')):
    print(mic)
    _gdf_shp = gdf_shp[gdf_shp['mic'] == mic]
    df = evaluate_emissions(mic, _gdf_shp, _taz_gdf, model_year, df_running_rates)
    df.to_csv(os.path.join(output_dir,fname+'.csv'), index=False)
    #try:
        #_gdf_shp = gdf_shp[gdf_shp['juris'] == city]
        #df = evaluate_emissions(_gdf_shp, _taz_gdf, model_year, df_running_rates)
