...
class GTFS_Utilities2(object):
    ...
    def __init__(self, gtfs_dir, start_time, end_time, service_ids="WEEKDAY"):
        """
        ...
        self.unique_stop_sequences_df = self.df_all_stops_by_trips.groupby(
            ["shape_id", "stop_sequence"]
        ).first()
        self.shapes = pd.read_csv(os.path.join(gtfs_dir, "shapes.txt"))
    ...
    def _get_trips_stop_times(self):
        ...
        stop_times_df = pd.read_csv(self.dir + "/stop_times.txt", index_col=None)
        stop_times_df = self._make_sequence_col(
            stop_times_df, ["trip_id", "stop_sequence"], "trip_id", "stop_sequence"
        )
        ...
        if stop_times_df["departure_time"].isnull().any():
            ...
            stop_times_df["departure_time_mins"] = stop_times_df.apply(
                self._convert_to_decimal_minutes, axis=1, args=("departure_time",)
            )
        ...
        trips_tw_df = trips_tw_df.merge(
            trips_df, "inner", left_on=["trip_id"], right_on=["trip_id"]
        )
    ...
    def _tph_from_frequencies(self):
        ...
        freq_df = pd.read_csv(self.dir + "/frequencies.txt", index_col=False)
        trips_df = pd.read_csv(self.dir + "/trips.txt", index_col=False)
        # Get the trips for this service_id
        trips_df = trips_df[trips_df["service_id"].isin(self.service_ids)]
        trips_df = trips_df[["trip_id", "shape_id"]]

        freq_df["start_time_secs"] = freq_df.apply(
            self._convert_to_seconds, axis=1, args=("start_time",)
        )
        freq_df["end_time_secs"] = freq_df.apply(
            self._convert_to_seconds, axis=1, args=("end_time",)
        )
        freq_df = freq_df.merge(
            trips_df, "left", left_on=["trip_id"], right_on=["trip_id"]
        )
        ...
        for row in freq_df.iterrows():
            for i in range(
                row[1].start_time_secs, row[1].end_time_secs, row[1].headway_secs
            ):
                my_list.append(
                    {
                        "trip_id": row[1].trip_id,
                        "shape_id": row[1].shape_id,
                        "departure_time_hrs": i / 3600,
                    }
                )

        first_departure = pd.DataFrame(my_list)
        first_departure = first_departure.groupby(["shape_id", "departure_time_hrs"])[
            "departure_time_hrs"
        ].count()
    ...
    def _get_stops_by_window(self):
        stops_by_window_df = self.df_all_stops_by_trips.loc[
            (self.df_all_stops_by_trips.departure_time_mins >= self.start_time)
            & (self.df_all_stops_by_trips.departure_time_mins < self.end_time)
        ]
    ...
    def _get_departure_times(self):
        ...
        first_departure = (
            self.df_stops_by_window.sort_values("stop_sequence", ascending=True)
            .groupby("trip_id", as_index=False)
            .first()
        )
        first_departure = first_departure.loc[(first_departure.stop_sequence == 1)]
        departure_times = {
            k: g["departure_time_mins"].tolist()
            for k, g in first_departure.groupby("shape_id")
        }
    ...
    def get_departure_times_by_ids(self, list_of_shape_ids):
        ...
        departure_time_dict = dict(
            (k, self.departure_times[k]) for k in (list_of_shape_ids)
        )
        departure_times = []
    ...
    def get_tph_rep_trip_id(self):
        ...
        a = self.df_all_stops_by_trips.merge(
            self.get_schedule_pattern_df(),
            "inner",
            left_on=["trip_id"],
        ...
        first_departure = (
            a.sort_values("stop_sequence", ascending=True)
            .groupby("trip_id", as_index=False)
            .first()
        )
    ...
    def get_tph_emme_rep_trip_id(self, results):
        ...
        # trips_tw_df = trips_tw_df.merge(trips_df, 'inner', left_on = ["trip_id"], right_on = ["trip_id"])
        df = self.df_all_stops_by_trips.loc[
            self.df_all_stops_by_trips.groupby("trip_id")["stop_sequence"].idxmin()
        ]

        df["trip_id"] = df["trip_id"].map(lambda x: x.strip())
        trip_ids_df["trip_id"] = trip_ids_df["trip_id"].map(lambda x: x.strip())
        first_departure = trip_ids_df.merge(
            df, "left", left_on=["trip_id"], right_on=["trip_id"]
        )
        # get the first stop for every trip
        # first_departure = a.sort_values('stop_sequence', ascending=True).groupby('trip_id', as_index=False).first()
        # this may not be necessary
        # first_departure = first_departure.loc[(first_departure.stop_sequence == 1)]
        first_departure = first_departure.groupby(["rep_trip", "departure_time_hrs"])[
            "departure_time_hrs"
        ].count()
        first_departure_df = pd.DataFrame(first_departure)
        first_departure_df.reset_index(level=0, inplace=True)
    ...
    def get_weighted_trips_per_hour(self):
        ...
        a = self.df_all_stops_by_trips.groupby(["trip_id", "shape_id"])[
            "departure_time_hrs"
        ].mean()
        b = pd.DataFrame(a)
        ...
        first_departure = b.groupby(["shape_id", "departure_time_hrs"])[
            "departure_time_hrs"
        ].count()
        first_departure_df = pd.DataFrame(first_departure)
        first_departure_df.reset_index(level=0, inplace=True)
    ...
    def get_weighted_tph_rep_trip_id(self):
        ...
        b.reset_index(level=0, inplace=True)
        b["departure_time_hrs"] = b["departure_time_hrs"].round().astype(int)
        first_departure = b.groupby(["rep_trip_id", "departure_time_hrs"])[
            "departure_time_hrs"
        ].count()
        first_departure_df = pd.DataFrame(first_departure)
        first_departure_df.reset_index(level=0, inplace=True)
    ...
    def get_schedule_pattern(self):
        ...
        stop_sequence_dict = {
            k: list(v)
            for k, v in self.df_all_stops_by_trips.groupby(["trip_id", "route_id"])[
                "stop_id"
            ]
        }
    ...
    def get_schedule_pattern_df(self):
        rows = []
        for route_id, trips in self.schedule_pattern_dict.items():
...
# test = GTFS_Utilities2(r"C:\Workspace\stefan\Transit_2050_OSM\gtfs\revised-2050", 0, 1439, 1)
# test = GTFS_Utilities2(r"C:\Stefan\scratch\gtfs\hold", 0, 1439, service_id = 1)
# am =test.get_tph_emme_rep_trip_id(r'C:\Workspace\stefan\Transit_2050_OSM\Outputs\MD\MD_detail.p')
# print ('done')
