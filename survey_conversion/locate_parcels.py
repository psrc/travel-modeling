...
def locate_person_parcels(person, parcel_df, df_taz):
    ...
    filter_dict_list = [{
        ...
        'person_filter': (-person['work_lng'].isnull()) & 
                         (person['workplace'] == "Usually the same location (outside home)")    # workplace is at a consistent location
        },
        {
...
def locate_trip_parcels(trip, parcel_df, df_taz):
    ...
    filter_dict_list = [
        ...
        {'parcel_filter': parcel_df['emptot_p'] > 0,
            'o_trip_filter': trip_results[opurp_field].isin(["Went to work-related place (e.g., meeting, second job, delivery)",
                                                             "Went to primary workplace","Went to other work-related activity"]),
            'd_trip_filter': trip_results[dpurp_field].isin(["Went to work-related place (e.g., meeting, second job, delivery)",
                                                             "Went to primary workplace","Went to other work-related activity"])},
        ...
        {'parcel_filter': ((parcel_df['empret_p'] > 0) | (parcel_df['empsvc_p'] > 0)),
            'o_trip_filter': trip_results[opurp_field].isin(["Conducted personal business (e.g., bank, post office)",
                                                             "Other appointment/errands (rMove only)"]),
            'd_trip_filter': trip_results[dpurp_field].isin(["Conducted personal business (e.g., bank, post office)",
                                                             "Other appointment/errands (rMove only)"])},
        ...
        {'parcel_filter': -parcel_df.isnull(),   # no parcel filter
            'o_trip_filter': trip_results[opurp_field].isin(["Went to religious/community/volunteer activity",
            ...
            'd_trip_filter': trip_results[dpurp_field].isin(["Went to religious/community/volunteer activity",
                                                             "Attended recreational event (e.g., movies, sporting event)",
                                                             "Went to a family activity (e.g., child's softball game)",
                                                             'Went to exercise (e.g., gym, walk, jog, bike ride)'])},
