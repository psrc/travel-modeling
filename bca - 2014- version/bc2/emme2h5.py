...
def emmemat2np(emmefullmatrix, dtype=np.float32):
    ...
    # Potentially unsafe. This correctly determines the dimension of
    # the matrix where there are holes in the middle of the zone
    # numbering system, but could cause problems if the zone index
    # exceeds the actual dimension. Problems should be rare if not
    # impossible.
    dim = max(mat.indices[1])
    npmat = np.empty((dim, dim), dtype)
    npmat[:] = np.NAN
    for x in mat.indices[0]:
        for y in mat.indices[1]:
            npmat[x-1][y-1] = mat.get(x,y)
    return(npmat)
        ...
        with tables.File(h5filepath, mode="a", filters=filters) as h5file:
            ...
            for mat in srcbank.matrices():
                ...
                if full_mat_name not in mats:
                    ...
                    if not dryrun:
                        a = h5file.createArray(h5grouppath,
                                               mat.name,
                                               emmemat2np(mat))
