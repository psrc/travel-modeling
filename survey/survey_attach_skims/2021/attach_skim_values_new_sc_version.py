...
run_root = r'C:\Workspace\sc_rtp_2018_final\soundcast'

input_dir = r'R:\e2projects_two\SoundCastDocuments\2014Estimation\New_06_20_16\inputs'
input_sep = ' '
output_dir = r'R:\e2projects_two\SoundCastDocuments\2014Estimation\with_2018_skims'

# save DAT file with MAM time format conversion
# Make sure directory exists before trying to save results there
output_dats = r'R:\e2projects_two\SoundCastDocuments\2014Estimation\with_2018_skims\dats'
...
def write_skims(df, skim_dict, otaz_field, dtaz_field, my_project, skim_output_file):
	...
	for i in xrange(len(df)):
		...
		rowresults['id'] = rowdata['id'].astype('str')
			...
			for skim_type in ['d','t','c']:
				...
				if skim_type == 'd':
					...
					if rowdata['mode code'] in ['bike','walk']:
						tod = bikewalk_tod
						rowresults[skim_type] = -1    # Recalculate based on time skims
						rowresults['tod_pulled'] = tod
						continue

				if rowdata['mode code'] in ['bike','walk'] and skim_type == 'c':
					tod = bikewalk_tod
					rowresults[skim_type] = 0    # Recalculate based on time skims
					rowresults['tod_pulled'] = tod
					continue
...
def fetch_skim(df_name, df, time_field, mode_field, otaz_field, dtaz_field, my_project, use_mode=False):
	...
	# Divide by 60 and round down to get hours (from minutes after 3am)
	# df[time_field] = df[time_field].astype('int')
	# hours = np.asarray(df[time_field].apply(lambda row: int(math.floor(row/60))))

	hours = np.asarray(df[time_field].astype('str').str[:-2].astype('int')) - 3
	# pd.DataFrame(hours).to_csv('hours.csv')
	# df['deptm'] = pd.to_datetime(df['deptm'])
	# hours = np.asarray(df['deptm'].apply(lambda row: row.hour).astype('int'))
	...
	df.to_csv('test_out2.csv',index=False)
...
def update_records(trip,person,tour):
	"""
	...
	for df in [trip_skim,person_skim,tour_skim,work_skim,school_skim]:
	# for df in [trip_skim]:
		# df['id'] = df['id'].astype('str')
		df['id'] = df['id'].astype('str')

	for df in [trip,person,tour]:
	# for df in [trip]:
		# df['id'] = df['id'].astype('str')
		df['id'] = df['id'].astype('str')
...
def hhmm_to_mam(df,field):
    ...
    hr = df[field].apply(lambda row: row.hour)
    minute = df[field].apply(lambda row: row.minute)
...
def main():
	...
	# Load data
	trip = pd.read_csv(input_dir + r'/tripP14.dat', delim_whitespace=True)
	tour = pd.read_csv(input_dir + r'/tourP14.dat', delim_whitespace=True)
	hh = pd.read_csv(input_dir + r'/hrecP14.dat', delim_whitespace=True)
	person = pd.read_csv(input_dir + r'/precP14.dat', delim_whitespace=True)
	...
	# trip = trip[trip['deptm'] >= 0]
	# trip = trip[trip['arrtm'] >= 0]
	# trip = trip[trip['endacttm'] >= 0]

	# for field in ['deptm','arrtm','endacttm']:
	#for field in ['deptm','arrtm']:
	...
	# Update records
	update_records(trip,person,tour)
	# update_records(tour)

	# Write results to h5
	# write_list = ['trip','prec','hrec','hday','pday']

	# copy non-updated files to the same output directory for consistency
	# for file in write_list:
	# 	if file not in ['prec','trip']:
	# 		copyfile(input_dir + r'\\' +file+version_tag+'.dat', output_dir + r'\\' +file+version_tag+'.dat')

	# dat_to_h5([output_dir + r'\\' +file+version_tag+'.dat' for file in ['trip','prec','hrec','hday','pday']])
