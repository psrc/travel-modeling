...
run_root = r'L:\RTP_2022\final_runs\sc_rtp_2018_final\soundcast'
os.chdir(run_root)
from EmmeProject import *

working_dir = os.getcwd()

input_dir = r'R:\e2projects_two\2022_base_year\2021_survey\daysim_format'
input_sep = ','
output_dir = r'R:\e2projects_two\2022_base_year\2021_survey\daysim_format\skims_attached'
...
def write_skims(df, skim_dict, otaz_field, dtaz_field, my_project, skim_output_file):
	...
	df.to_csv(os.path.join(output_dir,'before_bike_edits.csv'), index=False)
	...
	df.to_csv(os.path.join(output_dir,'after_bike_edits.csv'), index=False)

	# write results to a csv
	try:
		df.to_csv(os.path.join(output_dir,skim_output_file), index=False)
	except:
...
def fetch_skim(df_name, df, time_field, mode_field, otaz_field, dtaz_field, my_project, use_mode=False):
	...
	if use_mode:
		df['mode code'] = [mode_dict[use_mode] for i in range(len(df))]
	else:	
		df['mode code'] = [mode_dict[modes[i]] for i in range(len(df))]
	...
	skim_dict = {}
	for tod in tods:
	    contents = h5py.File(os.path.join(working_dir, r'inputs/model/roster', tod + '.h5'))
...
def process_person_skims(tour, person, hh):
	...
	person.to_csv(os.path.join(output_dir,'person21.csv'), index=False)
	#person[['hhno','pno','puwdepp',
	#'puwarrp','Work Mode', 'Work TAZ', 'School Arrival Time', 
	#'School Departure Time', 'School TAZ']].to_csv(os.path.join(output_dir,'person_skims.csv'), index=False)
...
def update_records(trip,tour,person):
	...
	# Load skim data
	trip_skim = pd.read_csv(os.path.join(output_dir, 'trip_skim_output.csv'))
	tour_skim = pd.read_csv(os.path.join(output_dir, 'tour_skim_output.csv'))
	person_skim = pd.read_csv(os.path.join(output_dir, 'person21.csv'))
	work_skim = pd.read_csv(os.path.join(output_dir, 'work_travel_skim_output.csv'))
	school_skim = pd.read_csv(os.path.join(output_dir, 'school_travel_skim_output.csv'))
	...
	for colname, skimname in trip_cols.items():
	    df.to_csv(os.path.join(output_dir,'testout.csv'), index=False)
	    ...
	    # export results
	    df.to_csv(os.path.join(output_dir, 'trip21.csv'), index=False) 
	...
	for colname, skimname in tour_cols.items():
	    ...
	    # export results
	    df.to_csv(os.path.join(output_dir, 'tour21.csv'), index=False) 
	...
	df = df.drop(['d','t'],axis=1)
	# export results
	df.to_csv(os.path.join(output_dir, 'person21.csv'), index=False) 
...
def main():
	...
	# Load data
	trip = pd.read_csv(input_dir + r'/_trip.tsv', sep='\t')
	tour = pd.read_csv(input_dir + r'/_tour.tsv', sep='\t')
	hh = pd.read_csv(input_dir + r'/_household.tsv', sep='\t')
	person = pd.read_csv(input_dir + r'/_person.tsv', sep='\t')
	...
	tour.to_csv(os.path.join(output_dir,'tour21.csv'), index=False)

	#for field in ['deptm','arrtm','endacttm']:
	#    trip = hhmm_to_mam(trip,field)
	trip.to_csv(os.path.join(output_dir,'trip21.csv'), index=False)
	...
	person = pd.read_csv(input_dir + r'/_person.tsv', delim_whitespace=True)
	person['id'] = person['hhno'].astype('str') + person['pno'].astype('str')
	...
	for file in write_list:
		df = pd.read_csv(os.path.join(output_dir, file+'21.csv'))
		df.to_csv(os.path.join(output_dir,'_'+file+'.tsv'), sep='\t', index=False)
